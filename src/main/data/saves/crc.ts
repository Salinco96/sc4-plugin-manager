// All credits to sebamarynissen and simmaster07
// https://github.com/sebamarynissen/sc4/blob/main/src/core/crc.ts
// https://github.com/sebamarynissen/sc4/blob/main/src/core/crc-table.ts
...
// # crc(buffer, buffer.length, offset = 0, iv = 0xffffffff)
...
export function crc32(buffer: Uint8Array, offset = 0, iv = IV) {
  ...
  const end = offset + Math.min(MAX, buffer.length - offset)

  // Now perform the *actual* checksum algorithm on the capped buffer.
  let crc = iv
  for (let i = offset; i < end; i++) {
    const index = uint(((crc >> 24) ^ buffer[i]) & 0xff)
